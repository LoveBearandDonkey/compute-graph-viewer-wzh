/* ══════════════════════════════════════════════════════════════════════════
   MC2 算子异常观测 · 配置关系与传播链
   ------------------------------------------------------------------------
   本文件 fork 自 js/config-relation-observer.js（openPangu 训练场景），改造为
   history.md 记录的 **vLLM + Ascend HCF 推理** 场景：MC2 通信融合算子在多 graph
   共享 aclOpExecutor 时污染 CCU mission，导致 NotifyWait 死锁。

   与训练版的三处结构性差别（也正是这个案例逼出来的新能力）：
     1. 新增「执行配置」域 —— enable_mc2 / mc2_comm_mode / cudagraph 捕获数。
        训练版的 stepper 只有并行维度与 MoE 维度，表达不了软件栈开关；而本案
        的根因恰恰只由这几个开关决定，且可推演（graph 数 = 污染面）。
     2. 血缘新增**资源节点**与 N:1「共享复用」边 —— 训练版 stages[].nodes 全是
        算子；aclOpExecutor / CCU mission 是**资源**，15 个 graph 指向同一个
        executor 这条边用 fusion/split/dispatch 都表达不了，另立 kind=shared。
     3. evidence 新增「对照实验」形态 —— 本案根因是靠 v15/v18 受控实验确认的，
        不是观测出来的，chart+metrics 两种形态装不下「触发 / 不触发」的对照。

   并行维度语义（vLLM 推理，见 history.md §二/§5.4）：
     TP=4，EP=4，**EP 组与 TP 组落在同一批 4 张卡上**（不是正交维度）。
     故 world = DP × PP × TP × CP，EP 不参与连乘 —— 由 preset.epOverlapsTp 开启，
     此时 tpIdx ≡ epIdx。训练版 openPangu 的 EP 与 DP 正交，两者不可混用。
     1 × 1 × 4 × 1 = 4 ✓                Node = 1（Ascend 950，8 NPU/node，用 4）

   ⚠️ 结构参数（层数 / 专家数）是**演示占位**：history.md 未记录 Hunyuan V3 的
      层数与专家数，只记录了并行度、算子名与故障时序。凡是真实证据都落在事件的
      evidence 里，四域结构只负责给传播关系提供可指认的对象。
   ══════════════════════════════════════════════════════════════════════════ */
(function (global) {
  "use strict";

  /* ── 模型预设 ─────────────────────────────────────────────────────────────
     故障环境：vLLM 0.1.dev14714 + Ascend HCF，HYV3VLForConditionalGeneration
     （0717_w8a8c8_mixed 权重），TP=4 / EP=4 落在同一批 4 张 Ascend 950 上。
     结构参数为演示占位（见文件头 ⚠️），并行度与卡数取自 history.md。 */
  const MODEL_PRESETS = {
    "hunyuan-v3-mc2": {
      id: "hunyuan-v3-mc2",
      label: "Hunyuan V3 · W8A8C8 mixed（vLLM 推理）",
      hidden: 4096,
      vocab: 128000,
      heads: 32,
      firstKDense: 1,       // L0 走 Dense MLP，其余是 MoE FFN
      dsaEvery: 4,
      mtpLayers: 0,
      denseIntermediate: 11008,
      moeIntermediate: 1408,
      // EP 组 = TP 组 = 同一批 4 张卡。开启后 world 不再乘 EP，且 tpIdx ≡ epIdx。
      epOverlapsTp: true,
      deck: { depthGap: 32, blockPostLayers: [0, 4, 9, 14, 19, 24, 29] },
      defaults: {
        totalLayer: 32,
        dp: 1, pp: 1, tp: 4, cp: 1,
        routedExpert: 64, topK: 8, sharedExpert: 1, ep: 4,
        totalRank: 4, node: 1,
        // ── 执行配置（本页新增域，故障环境实测值，见 history.md §一 / §5.4）──
        enableMc2: 1,                       // additional_config.enable_mc2
        commMode: "auto",                   // mc2_comm_mode: auto → 实走 CCU
        cudagraphMode: "full",              // FULL_PREFILL_AND_DECODE
        captureDecode: 7,                   // capture_sizes [1,2,4,8,16,32,64]
        capturePrefill: 8,                  // prefill [256…2048]
      },
    },
  };

  const DEFAULT_MODEL = "hunyuan-v3-mc2";

  function presetOf(config) {
    return MODEL_PRESETS[config?.model] || MODEL_PRESETS[DEFAULT_MODEL];
  }

  // EP 与 TP 是否共卡。共卡时 EP 退出 world 连乘，rank 编址里 tpIdx ≡ epIdx。
  function epOverlaps(config) {
    return !!presetOf(config).epOverlapsTp;
  }

  /* ── stepper 字段规格：min/max/step 与取值方式（pow2 = 按 2 的幂增减） ──
     enum 字段（commMode / cudagraphMode）不是数值 stepper，用 options 轮转；
     bool 字段（enableMc2）用 0/1 表示，读数区显示 on/off。 */
  const FIELD_SPECS = {
    ...
    /* ── 执行配置：本页新增（训练版没有这一组）────────────────────────────
       这四项就是 history.md §5.4 里那份 vLLM 配置。它们与并行维度不同——不改
       变「有哪些对象」，只改变「同一批对象之间会不会共享一份 runtime 资源」，
       因而是本案唯一能让传播链出现/消失的开关。 */
    enableMc2:    { label: "enable_mc2",     group: "exec", bool: true,
                    tip: "additional_config.enable_mc2：关掉则不走 MC2 通信融合算子，AllGatherMatmulV2 / AlltoAllMatmul 退回 matmul + HCCL 两段式，链路不成立。" },
    commMode:     { label: "comm_mode",      group: "exec",
                    options: ["auto", "ccu", "ai_cpu"],
                    tip: "mc2_comm_mode：auto 在 Ascend 950 上实走 CCU；ai_cpu 绕开 CCU jetty/channel 子系统（history.md §7.3 的临时规避，代价 5–15% 吞吐）。" },
    cudagraphMode:{ label: "cudagraph_mode", group: "exec",
                    options: ["full", "piecewise", "none"],
                    optionLabels: { full: "FULL_PREFILL_AND_DECODE", piecewise: "PIECEWISE", none: "NONE" },
                    tip: "cudagraph_mode：只有 FULL_PREFILL_AND_DECODE 会同时捕获 prefill 与 decode 两批 graph，把 graph 数推到十几个量级。" },
    captureDecode:{ label: "decode graph",   group: "exec", min: 0, max: 32, step: 1,
                    tip: "cudagraph_capture_sizes 的条目数，故障环境为 [1,2,4,8,16,32,64] 共 7 个。" },
    capturePrefill:{ label: "prefill graph", group: "exec", min: 0, max: 32, step: 1,
                    tip: "cudagraph_capture_sizes_prefill 的条目数，故障环境为 [256…2048] 共 8 个。" },
  };
  ...
  const FIELD_ORDER = {
    ...
    exec: ["enableMc2", "commMode", "cudagraphMode", "captureDecode", "capturePrefill"],
  };

  /* ── 触发判定 ─────────────────────────────────────────────────────────────
     history.md §5.2 的三组受控实验给出的判据，逐条对应一个必要条件：
       v15  3 graph + 共享 executor → it=0 即 507011  ✓ 触发
       v18  3 graph + 独立 executor → 2000 iters 0 错误 ✗
       v15  numGraphs=1             → 500 iters 0 错误 ✗
     即：MC2 开 ∧ 走 CCU ∧ graph 数 > 1 ∧ 捕获模式会 capture —— 缺一不触发。
     这是本页仿真态第一次能对配置改动做**预测**，而不只是复盘既成事实。 */
  function numGraphs(config) {
    if (config.cudagraphMode === "none") return 0;
    if (config.cudagraphMode === "piecewise") return config.captureDecode;
    return config.captureDecode + config.capturePrefill;
  }

  function mc2Verdict(config) {
    const graphs = numGraphs(config);
    const reasons = [];
    if (!config.enableMc2) reasons.push("enable_mc2 = false，不走 MC2 融合算子");
    if (config.commMode === "ai_cpu") reasons.push("comm_mode = ai_cpu，绕开 CCU 子系统");
    if (graphs <= 1) reasons.push(`graph 数 = ${graphs}，不存在跨 graph executor 复用`);
    return {
      graphs,
      triggered: reasons.length === 0,
      reasons,
      engine: config.commMode === "ai_cpu" ? "AICPU" : "CCU",
    };
  }
  ...
  function stepValue(field, value, direction) {
    ...
    // bool / enum 走轮转，不走数值加减
    if (spec.bool) return value ? 0 : 1;
    if (spec.options) {
      const index = Math.max(0, spec.options.indexOf(value));
      const next = (index + direction + spec.options.length) % spec.options.length;
  ...
  // 读数区显示值：bool → on/off，enum → 长标签（若有）
  function displayValue(field, value) {
    const spec = FIELD_SPECS[field];
    if (spec.bool) return value ? "on" : "off";
    if (spec.options) return spec.optionLabels?.[value] || String(value);
    return String(value);
  }
  ...
  function validate(config) {
    ...
    const overlap = epOverlaps(config);
    ...
    // EP 与 TP 共卡时，EP 组必须正好铺满 TP 组的那几张卡，否则 rank 编址无解
    if (overlap && ep !== tp) {
      errors.push(`EP ${ep} 与 TP ${tp} 共用同一批卡，两者必须相等（vLLM TP=EP=4）`);
    }
    const world = overlap ? dp * pp * tp * cp : dp * pp * tp * cp * ep;
    if (world !== totalRank) {
      const formula = overlap
        ? `DP${dp}×PP${pp}×TP${tp}×CP${cp}（EP${ep} 与 TP 共卡，不参与连乘）`
        : `DP${dp}×PP${pp}×TP${tp}×CP${cp}×EP${ep}`;
      errors.push(`${formula} = ${world}，与 Total Rank ${totalRank} 不符`);
    }
    if (totalRank % node !== 0) {
  ...
  // EP 共卡时 EP 不参与 world 连乘，也就不能被拿去配平 —— 它由 TP 带着走。
  function worldFields(config) {
    return epOverlaps(config)
      ? PARALLEL_FIELDS.filter((field) => field !== "ep")
      : PARALLEL_FIELDS;
  }

  function parallelWorld(config) {
    return worldFields(config).reduce((product, field) => product * config[field], 1);
  }
  ...
  function isAllowedParallelValue(field, value, config) {
    ...
    // 共卡模式下 TP 的取值同时是 EP 的取值，必须能整除专家数
    if (epOverlaps(config) && field === "tp" && config.routedExpert % value !== 0) return false;
    return true;
  }
  ...
  function fitParallelWorld(config, target, anchor) {
    const usable = new Set(worldFields(config));
    const candidates = ["dp", "ep", "tp", "cp", "pp"]
      .filter((field) => field !== anchor && usable.has(field));
  ...
  function reconcile(config, anchor) {
    /* EP 与 TP 共卡：两者是同一组卡的两种切法，必须同值。谁被用户拨动谁做主，
       另一个跟随；跟随值若不能整除专家数，由下面的 routedExpert 约束再收敛。 */
    if (epOverlaps(config) && config.ep !== config.tp) {
      if (anchor === "tp") config.ep = config.tp;
      else config.tp = config.ep;
    }
    ...
    if (config.routedExpert % config.ep !== 0) {
      if (anchor === "ep" || (epOverlaps(config) && anchor === "tp")) {
    ...
    // 上一步可能把 ep 压小，共卡模式下 tp 要跟回来
    if (epOverlaps(config)) config.tp = config.ep;
  ...
  function derive(config) {
    ...
    const overlap = epOverlaps(config);
    const verdict = mc2Verdict(config);
    ...
    /* rank 编址：stage 最外，dp 次之，ep 最内（TP/CP 内联在最内层）。
       共卡模式下 TP 不再占一层地址位 —— 第 p 个 EP rank 就是第 p 个 TP rank，
       内层只剩 CP，因此 ranksPerEp = cp 而不是 tp*cp。 */
    const ranksPerEp = overlap ? cp : tp * cp;
    ...
    const rankOfCoords = (stage, dpIdx, epIdx, tpIdx = 0, cpIdx = 0) =>
      overlap
        ? rankOf(stage, dpIdx, epIdx, cpIdx)      // tpIdx ≡ epIdx，已在地址里
    ...
    function coordsOfRank(rank) {
      ...
      const tpIdx = overlap ? epIdx : inner % tp;
      const cpIdx = overlap ? inner : Math.floor(inner / tp);
    ...
    return {
      ...
      counts: {
        ...
        graphs: verdict.graphs, engine: verdict.engine,
      },
  ...
  function buildStepper(field, value, onChange) {
    ...
    // bool / enum 的读数是文字而非数字，读数区要放宽并改左对齐
    if (spec.bool || spec.options) wrap.classList.add("cro-stepper--text");

    const label = document.createElement("span");
    label.className = "cro-stepper__label";
    label.textContent = spec.label;
    if (spec.tip) label.dataset.tip = spec.tip;
    ...
    readout.textContent = displayValue(field, value);
  ...
  /* ── 触发判定条 ───────────────────────────────────────────────────────────
     挂在「执行配置」组下方。这是本页与训练版最实质的差别：训练版的四域是
     「这套配置长什么样」的静态查询，改 stepper 只换对象；这里改 stepper 会翻转
     一个**结论** —— 同一批对象、同一份代码，会不会踩到 CCU mission 污染。
     判据见 mc2Verdict()，三组受控实验的必要条件。 */
  function renderVerdict(topology) {
    const host = document.getElementById("croExecVerdict");
    if (!host) return;
    const v = topology.verdict;
    host.dataset.state = v.triggered ? "triggered" : "safe";
    host.replaceChildren();

    const tag = document.createElement("span");
    tag.className = "cro-exec-verdict__tag";
    tag.textContent = v.triggered ? "触发" : "不触发";

    const text = document.createElement("p");
    text.className = "cro-exec-verdict__text";
    text.textContent = v.triggered
      ? `${v.graphs} 个 graph 共享同一个 aclOpExecutor，走 ${v.engine} 通信引擎 —— 与故障环境同构，CCU mission 会被跨 graph 覆盖。`
      : `${v.reasons.join("；")}。链路在此断开，不会发生 CCU mission 污染。`;
  ...
  function createController(options = {}) {
    ...
    function emit() {
      ...
      readouts.forEach((el, field) => { el.textContent = displayValue(field, config[field]); });
      // 执行配置那一组：把「当前这套配置会不会触发」直接写在组下面（见 renderVerdict）
      renderVerdict(topology);
  ...
  function deckConfigFrom(topology) {
    ...
    // 整份对象直接当 config 传给 pattern（不是按 id 查 pattern 内置预设），
    // 所以 id 只是个标识，层数 / 专家数全部由本页 topology 覆写。
    return {
      id: preset.id,
      label: preset.label,
      layerCount: counts.totalLayer,
      depthGap: preset.deck.depthGap,
      frontLayer: Math.floor(lastLayer / 2),   // 正视图默认停在中间层
  ...
  /* ── 对照实验（本页新增的第 4 种证据形态）─────────────────────────────────
     line / bars / stack 三种都在回答「多少」，前提是这件事已经发生、只等被观测。
     但 MC2 的根因不是观测出来的，是**跑出来的**：改一个变量、看触发与否。它没有
     量纲、没有阈值，也不该有坐标轴 —— 硬套柱状图会让人去比 2000 iters 和 500
     iters 谁大，而那恰恰是无意义的对比。所以另立一种：每行一组实验，列出
     「配置 / 结果」，命中的那组描红。
       spec = { kind:"control", runs:[{ name, setup, result, trigger }], note } */
  function chartControl(spec) {
    const wrap = document.createElement("div");
    wrap.className = "cro-control";
    (spec.runs || []).forEach((run) => {
      const row = document.createElement("div");
      row.className = `cro-control__row${run.trigger ? " is-trigger" : ""}`;

      const name = document.createElement("span");
      name.className = "cro-control__name";
      name.textContent = run.name;

      const setup = document.createElement("span");
      setup.className = "cro-control__setup";
      setup.textContent = run.setup;

      const result = document.createElement("span");
      result.className = "cro-control__result";
      result.textContent = run.result;

      row.append(name, setup, result);
      wrap.appendChild(row);
    });
    return wrap;
  }

  const CHART_BUILDERS = { line: chartLine, bars: chartBars, stack: chartStack, control: chartControl };
  ...
  function completeLineage(data) {
    const auto = buildLineageEdges(data.stages);
    if (data.sharedEdges?.length) {
      // 同一对相邻层里，共享边取代自动顺序边：两条并存会让「15→1」读成「15→15」
      const suppressed = auto.filter((edge) => `${edge.fromStage}>${edge.toStage}` !== data.sharedPair);
      data.edges = suppressed.concat(data.sharedEdges);
    } else {
      data.edges = auto;
    }
    return data;
  ...
  function buildIncidentLineage(event, topology) {
    ...
    /* 逐层节点覆写（本页新增）：MC2 案例的 GE / Runtime / Kernel 三层不是靠算子
       op 推导得出的 —— GE 层是 15 个 graph、Runtime 层有 aclOpExecutor 这种
       **资源**节点。给了 spec.nodes[stage] 就照抄，没给才回退到 LINEAGE_LOWERING。
       节点条目支持 { name, detail, kind }，kind:"resource" 渲染成资源样式。 */
    const nodeId = (stageId, index) => `event:${event.id}:${stageId}:${layerKey}:${nodeKey}:${index}`;
    ...
    const data = {
      ...
      stages: LINEAGE_STAGE_META.map((meta) => {
        ...
        const override = isAvailable ? spec.nodes?.[meta.id] : null;
        const entries = override?.length
          ? override.map((entry) => (typeof entry === "string" ? { name: entry } : entry))
          : (isAvailable && namesByStage[meta.id]?.length ? namesByStage[meta.id] : ["本事件未定位到该层"])
              .map((name) => ({ name }));
        return {
          ...meta,
          available: isAvailable,
          roles: [],
          // 该层节点数多于展示条目时，由 spec.nodes 里最后一条自己写「… 共 N 个」
          nodes: entries.map((entry, index) => ({
            name: entry.name,
            id: isAvailable ? nodeId(meta.id, index) : "",
            detail: entry.detail || (isAvailable ? detailByStage[meta.id] : "缺少该层的直接定位信息"),
            kind: entry.kind || "op",
            roles: [],
          })),
        };
    ...
    /* ── 共享复用边（本页新增的第 5 种 edge kind）─────────────────────────
       fusion / split / lowering / dispatch 四种都在描述「一个东西变成了另一个
       东西」，是**正常的**编译变换。而 15 个 graph 指向同一个 aclOpExecutor 描述
       的是「本不该共享的东西被共享了」—— 它不是变换，是缺陷本身，所以另立
       kind=shared，用红虚线画，并顶掉这两层之间自动生成的顺序边。 */
    if (spec.shared) {
      const { fromStage, toStage, toIndex = 0, label = "共享复用" } = spec.shared;
      const from = data.stages.find((entry) => entry.id === fromStage);
      const to = data.stages.find((entry) => entry.id === toStage);
      if (from?.available && to?.available && to.nodes[toIndex]) {
        const sources = from.nodes.filter((node) => node.id);
        // N 条边同一个终点，标签只挂中间那条：三份「共享复用」叠在一处只是噪声
        const labelled = Math.floor(sources.length / 2);
        data.sharedEdges = sources.map((node, index) => ({
          id: `${node.id}>${to.nodes[toIndex].id}`,
          from: node.id,
          to: to.nodes[toIndex].id,
          fromStage, toStage,
          kind: "shared",
          label: index === labelled ? label : "",
        }));
        data.sharedPair = `${fromStage}>${toStage}`;
      }
    }
  ...
  /* ── 事件链 ───────────────────────────────────────────────────────────────
     七个事件按**用户真实排查顺序**排列，不是因果顺序 —— 根因（mc2-root）排在
     第 5 位，前四步是从表象一路缩小到「不是模型、不是算子实现」的定界过程。
     这与训练版两个案例的编排一致：排查是从看得见的地方开始的。

     全部数字与日志片段取自 history.md，逐条对应：
       ① §三 Run#4 的 server_mxfp8_all_withmc2.log 渐进恶化表
       ② §三 四次复现证据链的共同特征
       ③ §三 Run#4 的 4 rank 故障时序（cqeStatus）
       ④ §三 Run#1 + §四 关键定界证据
       ⑤ §5.1 根因 + §5.2 复现实验（v15 / v18 / 单 graph）
       ⑥ §5.4 与故障环境 vLLM 的关联
       ⑦ §7 修复方案总览 */

  // Run#4 里唯一 cqeStatus=0x8000 的那张卡。它最后报错，却是真凶。
  const CULPRIT_RANK = 1;

  const INCIDENT_GROUPS = [
    {
      id: "problem-1",
      codePrefix: "MC2",
      name: "MC2 · CCU mission 资源污染",
      // 本案的对象范围是「每一层的 MC2 融合算子 × 这 4 张卡」——不像训练版那样
      // 收敛到某一层某个专家，因为污染的是跨 graph 共享的一份 runtime 资源。
      context: { layers: "all", segments: ["moe"], ranks: "tp-group", epRanks: "all" },
      bridge: (event) => `${event.title}（${event.time}）· 沿“${event.path}”传导`,
      events: [
        /* ①  表象层：先看见的是「越来越慢」，不是报错 ───────────────────── */
        {
          id: "mc2-degrade", time: "14:07", dimension: "耗时 · 趋势",
          title: "forward_time 三分钟恶化 85 倍",
          focus: { kind: "rank", rank: CULPRIT_RANK },
          origin: { ranks: [CULPRIT_RANK] },
          propagation: { segments: ["moe"] },
          victim: { ranks: "tp-group" },
          lineage: {
            operator: "AllGatherMatmulV2（AllGather + MatMul 融合）",
            segment: "moe", bar: "attn", rank: CULPRIT_RANK,
            stages: ["runtime"],
            nodes: {
              runtime: [
                { name: "AllGatherMatmulV2 task", detail: "stream 61 · 每次 replay 都走同一份 CCU mission 引用" },
              ],
            },
          },
          conclusion: "恶化是线性累积的，不是抖动 —— 说明被污染的是一份会被反复复用的资源，而不是某次调度的偶发争抢。",
          root: "每次 graph replay 都在同一份被污染的 CCU mission 上等待，等待时间逐轮叠加",
          path: "executor 复用 → CCU mission 引用错位 → NotifyWait 逐轮变长",
          impact: "三分钟内单次 forward 从 2s 涨到 170s，随后完全 hang",
          evidence: {
            chart: {
              kind: "line", title: "forward_time 线性恶化（server_mxfp8_all_withmc2.log）", unit: " s",
              x: ["14:07:00", "14:07:20", "14:08:47", "14:09:48"],
              threshold: { value: 3, label: "稳态上界 · ~2s" },
              values: [1.97, 21.9, 107, 168],
              mark: { index: 3, label: "168s · 完全 hang", tone: "danger" },
              note: "若是 race condition，慢的应该是零星几次；这里每一轮都比上一轮慢，是资源被逐次污染的形状。",
            },
            metrics: [
              { label: "恶化倍数", value: "1x → 85x", tone: "danger" },
              { label: "观察窗口", value: "约 3 分钟" },
              { label: "形态", value: "线性累积，非瞬时 race" },
              { label: "此刻的报错", value: "无 —— 服务还在返回" },
            ],
          },
        },
        ...
        /* ②  报错层：用户唯一看得见的那一句 ─────────────────────────────── */
        {
          id: "mc2-hang", time: "20:26:52.919", dimension: "通信 · 耗时",
          title: "StreamSynchronize 超时 → 507011",
          focus: { kind: "rank", rank: CULPRIT_RANK },
          origin: { ranks: [CULPRIT_RANK] },
          propagation: { segments: ["moe"] },
          victim: { ranks: "tp-group" },
          lineage: {
            operator: "AllGatherMatmulV2_5cc364e0…_64_mix_aic",
            segment: "moe", bar: "attn", rank: CULPRIT_RANK,
            stages: ["runtime", "kernel"],
            nodes: {
              runtime: [
                { name: "model execute task（model_id=55）", detail: "stream 61 · taskType=14 NOTIFY_WAIT · NotifyId=13224" },
                { name: "aclrtStreamSynchronize", detail: "timeout=-1ms → 507011，Python 侧只看得到这一句" },
              ],
              kernel: [
                { name: "AllGatherMatmulV2_…_64_mix_aic", detail: "FUSION_KERNEL（taskType=107）· tilingKey=64 MIX_AIC" },
              ],
            },
            originNode: { stage: "kernel", index: 0 },
            victimStage: "runtime",
          },
          conclusion: "这是整条链上唯一暴露给业务侧的信息，而它指向的是通信同步 —— 按它去查 HCCL 就会走反方向。",
          root: "notify wait 迟迟等不到，stream 同步一路超时冒到 APIServer",
          path: "notify wait 阻塞 → StreamSynchronize 超时 → EngineCore RPC → APIServer RuntimeError",
          impact: "TP0~TP3 四个进程全部退出，业务不可用，需重启",
          evidence: {
            chart: {
              kind: "bars", title: "四次复现的报错特征命中次数", unit: " / 4 次",
              items: [
                { label: "sqe_type=7（notify wait）", value: 4, tone: "danger" },
                { label: "errType=0x20（sq sw status error）", value: 4, tone: "danger" },
                { label: "stream_id=61", value: 4 },
                { label: "错误码 507011", value: 4 },
              ],
              note: "跨 3 个日期、2 种算子、4 次复现，四项特征全中 —— 这不是偶发，是同一条确定的代码路径。",
            },
            metrics: [
              { label: "Python 侧可见", value: "仅 RuntimeError / 507011" },
              { label: "复现次数", value: "4 次 · 跨 3 个日期" },
              { label: "涉及算子", value: "AllGatherMatmulV2、AlltoAllMatmul" },
              { label: "误导方向", value: "报错指向 HCCL 同步", tone: "warning" },
            ],
          },
        },
        ...
        /* ③  集群定界：首报的不是真凶 ───────────────────────────────────── */
        {
          id: "mc2-culprit", time: "20:26:52.916", dimension: "集群 · 定界",
          title: "四卡全报，只有 rank 1 是真凶",
          focus: { kind: "rank", rank: CULPRIT_RANK },
          origin: { ranks: [CULPRIT_RANK] },
          victim: { ranks: "peers" },
          lineage: {
            operator: "AllGatherMatmulV2 · device 端 CQE 上报",
            segment: "moe", bar: "attn", rank: CULPRIT_RANK,
            stages: ["runtime", "kernel"],
            nodes: {
              runtime: [
                { name: "ProcessStarsOneElementInRingBuffer（type=107）", detail: "device 侧主动上报，不是 host 端超时判定" },
              ],
              kernel: [
                { name: "CQE · cqeStatus=0x8000", detail: "rank 1 独有；rank 0/2/3 的 cqeStatus 均为 0" },
              ],
            },
            originNode: { stage: "kernel", index: 0 },
          },
          conclusion: "按报错时间排序会把 rank 2 排成首犯 —— 真正的判据是 cqeStatus，而不是谁先喊。",
          root: "rank 1 的 device 端主动上报 CQE 异常，其余三张卡是被它拖住后超时",
          path: "rank 1 CQE 0x8000 → 同组三卡 notify 等待超时 → 四卡同步退出",
          impact: "四张卡都报 EZ9999，按报错数或报错顺序定位都会得出错误结论",
          evidence: {
            chart: {
              kind: "bars", title: "Run#4 四张卡的 cqeStatus（按报错先后排列）", unit: "",
              items: [
                { label: "rank 2 · 20:26:52.219（首报）", value: 0 },
                { label: "rank 3 · 20:26:52.388", value: 0 },
                { label: "rank 1 · 20:26:52.916", value: 32768, tone: "danger" },
                { label: "rank 0 · 20:26:53.709", value: 0 },
              ],
              note: "0x8000 = 32768。四张卡里只有一张带着 device 端异常状态，其余三张的 CQE 是干净的 —— 它们只是在等它。",
            },
            metrics: [
              { label: "首报", value: "rank 2（+0ms）" },
              { label: "真凶", value: "rank 1（+697ms）", tone: "danger" },
              { label: "判据", value: "cqeStatus=0x8000，非报错时间" },
              { label: "若按时间排", value: "结论正好排反", tone: "warning" },
            ],
          },
        },
        ...
        /* ④  硬件定界：CCU，不是 AI Core ────────────────────────────────── */
        {
          id: "mc2-ccu", time: "+0ms", dimension: "硬件 · 定界",
          title: "只有 CCU 报错，AIC / AIV 全零",
          focus: { kind: "segment", segment: "moe", bar: "a2a_combine", deckNode: "a2a_combine" },
          origin: { ranks: [CULPRIT_RANK], segments: ["moe"] },
          propagation: { epRanks: "all" },
          victim: { ranks: "tp-group", layers: "all" },
          lineage: {
            operator: "AlltoAllMatmul / AllGatherMatmulV2（MIX_AIC，crossCoreSync=1）",
            segment: "moe", bar: "a2a_combine", rank: CULPRIT_RANK,
            stages: ["ge", "runtime", "kernel"],
            nodes: {
              ge: [
                { name: "AllGatherMatmulV2", detail: "MC2 融合：AllGather 与 MatMul 合成单个 FUSION_KERNEL" },
              ],
              runtime: [
                { name: "HCCL MC2 executor", detail: "opExpansionMode=5 · commEngine=5（CCU 通信引擎）" },
                { name: "CCU mission 5", kind: "resource", detail: "WaitGroup / AllGather / AlltoAll 通信 mission，四次复现全部落在这个 id" },
              ],
              kernel: [
                { name: "…_64_mix_aic", detail: "tilingKey=64 · kernelType=MIX_AIC · taskRation=1:2" },
                { name: "missionError · CCUM Execute Error(0x09)", detail: "Mission Task Killed(0x02)，随后 clean ccu task kill state 失败" },
              ],
            },
            originNode: { stage: "runtime", index: 1 },
            victimStage: "kernel",
          },
          conclusion: "故障起源在 CCU 通信单元。Run#2 里 AIC 的 GM 越界（errcode=264）是被 CCU 拖垮后的连带表现，不是另一个 bug。",
          root: "CCU mission 5 执行失败，四张卡在 WaitGroup 上互等",
          path: "CCU mission 5 失败 → WaitGroup sem[337] 互等 → MIX 算子整体失败",
          impact: "AIC/AIV 只是被波及；照着 errcode=264 去查算子实现会查空",
          evidence: {
            chart: {
              kind: "bars", title: "Run#1 的三类单元错误标志（device_error_proc_c.cc:847）", unit: "",
              items: [
                { label: "aicError（AI Cube 计算单元）", value: 0 },
                { label: "aivError（AI Vector 计算单元）", value: 0 },
                { label: "ccuError（CCU 通信单元）", value: 1, tone: "danger" },
              ],
              note: "计算单元一个都没错，只有通信单元错 —— 这一条就把「算子算错了」整条方向排除掉了。",
            },
            metrics: [
              { label: "missionId", value: "5（四次复现一致）", tone: "danger" },
              { label: "WaitGroup", value: "sem[337] · mask[0x000e] · rankIds[1,2,3]" },
              { label: "Run#2 的 AIC 越界", value: "errcode=264，连带表现", tone: "warning" },
              { label: "comm_flag", value: "2（通信类算子异常）" },
            ],
          },
        },
        ...
        /* ⑤  根因：靠受控实验确认，不是靠观测 ───────────────────────────── */
        {
          id: "mc2-root", time: "-3min", dimension: "配置 · 软件栈",
          title: "15 个 graph 共享同一个 aclOpExecutor",
          focus: { kind: "segment", segment: "moe", bar: "attn", deckNode: "attention_core" },
          origin: { segments: ["moe"], layers: "all" },
          propagation: { ranks: [CULPRIT_RANK] },
          victim: { ranks: "tp-group" },
          lineage: {
            operator: "aclnnAllGatherMatmulV2GetWorkspaceSize",
            segment: "moe", bar: "attn", rank: CULPRIT_RANK,
            stages: ["model", "fx", "ge", "runtime", "kernel"],
            nodes: {
              model: [
                { name: "Attention qkv 投影（TP=4）", detail: "模型语义层完全正常 —— 根因不在这里" },
              ],
              fx: [
                { name: "call_function.npu_all_gather_matmul", detail: "框架捕获结果正常" },
              ],
              // GE 层就是那 15 个 capture 出来的 graph。列 3 个代表 + 1 条汇总，
              // N:1 的「N」端要能被看见，否则共享边读起来只是一根普通的线。
              ge: [
                { name: "graph #0 · decode bs=1", detail: "第一个 capture 的 graph，executor 在这里首次绑定 CCU mission" },
                { name: "graph #6 · decode bs=64", detail: "故障 kernel 所在的 graph（_64_mix_aic）" },
                { name: "graph #7…#14 · prefill 256→2048", detail: "其余 8 个 prefill graph，共 15 个" },
              ],
              runtime: [
                { name: "aclOpExecutor（缓存复用）", kind: "resource", detail: "15 个 graph capture 时拿到的是同一个 executor —— 缺陷就在这里" },
                { name: "CCU mission / notify id", kind: "resource", detail: "被后续 graph 的 capture 覆盖，replay 时引用到错误的 notify" },
              ],
              kernel: [
                { name: "NotifyWait（sqe_type=7）", detail: "引用到错误 notify id，等不到 → device 端上报 CQE 0x8000" },
              ],
            },
            originNode: { stage: "runtime", index: 0 },
            victimStage: "kernel",
            // N:1 共享复用边：15 个 graph → 1 个 executor。顶掉 ge→runtime 的
            // 自动顺序边，否则会读成「每个 graph 各有一个 executor」。
            shared: { fromStage: "ge", toStage: "runtime", toIndex: 0, label: "15 个 graph 共享 1 个 executor" },
          },
          conclusion: "根因在 opapi 层：executor 被跨 graph 复用，把第一个 graph 的 CCU mission 状态覆盖掉了。模型与框架层全程无辜。",
          root: "aclnnAllGatherMatmulV2GetWorkspaceSize 返回了缓存的 aclOpExecutor",
          path: "多 graph capture → 共享 executor → CCU mission 引用被覆盖 → replay 时 NotifyWait 死锁",
          impact: "凡是多 graph + MC2 + CCU 的组合都会踩到，与具体算子、具体层无关",
          evidence: {
            chart: {
              kind: "control", title: "受控实验：只改 executor 是否共享",
              runs: [
                { name: "v15", setup: "3 graph · 共享 execs[gi]", result: "it=0 即 507011", trigger: true },
                { name: "v18", setup: "3 graph · 每 graph 独立 execs[gg][gi]", result: "2000 iters 0 错误" },
                { name: "v15 · 单 graph", setup: "numGraphs=1 · 共享写法不生效", result: "500 iters 0 错误" },
              ],
              note: "三组只差一个变量。共享即触发、独立即不触发、单 graph 无从共享 —— 根因由此闭合，不需要再解释别的现象。",
            },
            metrics: [
              { label: "故障环境 graph 数", value: "15（decode 7 + prefill 8）", tone: "danger" },
              { label: "复现耗时", value: "it=0 就崩" },
              { label: "对照组", value: "v18 跑满 2000 iters 无错" },
              { label: "v15 进程结局", value: "exit 139（Segmentation fault）", tone: "danger" },
            ],
          },
        },

        /* ⑥  配置耦合：本页主场 —— 改 stepper 会翻转结论 ────────────────── */
        {
          id: "mc2-config", time: "静态", dimension: "配置 · 关系",
          title: "四个开关同时成立才会触发",
          focus: { kind: "rank", rank: CULPRIT_RANK },
          origin: { ranks: "tp-group" },
          propagation: { segments: ["moe"] },
          victim: { layers: "all", segments: ["moe"] },
          lineage: {
            operator: "vLLM cudagraph capture · MC2 通信引擎选择",
            segment: "moe", bar: "attn", rank: CULPRIT_RANK,
            stages: ["ge", "runtime"],
            nodes: {
              ge: [
                { name: "cudagraph_mode=FULL_PREFILL_AND_DECODE", detail: "prefill 与 decode 两批一起 capture，graph 数被推到 15" },
              ],
              runtime: [
                { name: "HCCL comm（TP=4 / EP=4）", kind: "resource", detail: "15 个 graph 共享同一个通信域" },
                { name: "mc2_comm_mode=auto → CCU", detail: "改成 ai_cpu 即绕开 CCU jetty/channel 子系统" },
              ],
            },
            originNode: { stage: "ge", index: 0 },
          },
          conclusion: "右侧「执行配置」四项就是这条链的开关组：任意一项改动都会让下面的判定条从「触发」翻成「不触发」。",
          root: "enable_mc2 ∧ comm_mode 走 CCU ∧ graph 数 > 1 —— 缺一不成立",
          path: "capture 配置 → graph 数 → 共享面 → CCU mission 污染",
          impact: "同一份代码，换一组配置就完全不复现 —— 这也是它难以在别处复现的原因",
          evidence: {
            chart: {
              kind: "stack", title: "15 个 cudagraph 的构成", unit: " 个 graph",
              items: [
                { label: "decode · capture_sizes [1,2,4,8,16,32,64]", value: 7 },
                { label: "prefill · [256…2048]", value: 8, tone: "warning" },
              ],
              note: "graph 数就是污染面：每多一个 capture，就多一次覆盖同一个 executor 的机会。把它降到 1，链路直接断开。",
            },
            metrics: [
              { label: "enable_mc2", value: "true", tone: "danger" },
              { label: "mc2_comm_mode", value: "auto → 实走 CCU", tone: "danger" },
              { label: "并行配置", value: "TP=4 / EP=4（同一批 4 卡）" },
              { label: "HCCL comm", value: "15 个 graph 共享 1 个" },
            ],
          },
        },
        ...
        /* ⑦  修复：四条落在四个责任方 ───────────────────────────────────── */
        {
          id: "mc2-fix", time: "—", dimension: "修复 · 分层",
          title: "四条修复，四个责任方",
          focus: { kind: "segment", segment: "moe", bar: "attn", deckNode: "attention_core" },
          origin: { segments: ["moe"] },
          victim: { ranks: "tp-group", layers: "all" },
          lineage: {
            operator: "aclnnAllGatherMatmulV2GetWorkspaceSize（修复点）",
            segment: "moe", bar: "attn", rank: CULPRIT_RANK,
            stages: ["ge", "runtime"],
            nodes: {
              ge: [
                { name: "每个 graph capture 独立取 executor", detail: "业务侧（vLLM / torch_npu）不缓存 executor" },
              ],
              runtime: [
                { name: "aclOpExecutor（每次调用新建）", kind: "resource", detail: "opapi 侧根因修复：graph capture 模式下不复用缓存" },
                { name: "CCU mission 绑定 model_id", kind: "resource", detail: "HCCL 侧防御性修复：检测到跨 graph 复用即拒绝执行" },
              ],
            },
            originNode: { stage: "runtime", index: 0 },
          },
          conclusion: "根因修复在 opapi 侧且无性能损失；切 AICPU 只是驱动修复前的临时规避，要付 5–15% 吞吐。",
          root: "graph capture 模式下每次返回独立的 aclOpExecutor",
          path: "opapi 根因修复 → HCCL 兜底 → 业务侧确认 → 配置临时规避",
          impact: "修复后用 v15 验证不再触发、v18 验证功能正常",
          evidence: {
            chart: {
              kind: "bars", title: "四条修复各自的吞吐代价", unit: "%",
              items: [
                { label: "opapi · 独立 executor（根因）", value: 0 },
                { label: "HCCL · mission 绑 model_id（防御）", value: 0 },
                { label: "torch_npu · 按 graph 隔离缓存", value: 0 },
                { label: "配置 · 切 AICPU（临时）", value: 10, tone: "warning" },
              ],
              note: "只有临时规避要付代价。它的价值是「驱动修复到位前先让业务跑起来」，不是长期方案。",
            },
            metrics: [
              { label: "根因修复位置", value: "opapi 层（§7.1）" },
              { label: "验证方式", value: "v18 跑 2000 iters 0 错误" },
              { label: "临时规避", value: "mc2_comm_mode: \"ai_cpu\"" },
              { label: "规避代价", value: "AICPU 比 CCU 慢 5–15%", tone: "warning" },
            ],
          },
        },
      ],
  ...
  function boot() {
    ...
    controller.mount(document.getElementById("croExecSteppers"), "exec");
    ...
    function expandIncidentValues(spec, key, topology) {
      ...
      /* 本页的 rank 记号（训练版那套 stage / ep-stage / ep-stage-peers 是按
         2048 卡 4 个 PP stage 写的，推理场景只有一批 4 卡，用不上）：
           "tp-group" —— TP=EP 共卡的那一组，也就是本次故障的全部 4 张卡
           "peers"    —— 除真凶 rank 1 外的其余 3 张（故障传播方） */
      if (key === "ranks" && (value === "tp-group" || value === "peers")) {
        const group = Array.from({ length: topology.counts.totalRank }, (_, i) => i);
        return value === "peers" ? group.filter((rank) => rank !== CULPRIT_RANK) : group;
      }
    ...
    function paintIncidentLineageEdges(shell, svg, data, initialSelection = new Set()) {
      ...
      data.edges.forEach((edge) => {
        ...
        label.classList.add("cro-incident-lineage__edge-label", `is-${edge.kind}`);
    ...
    function renderIncidentLineage(host, event, topology) {
      ...
      data.stages.forEach((stage, stageIndex) => {
        ...
        stage.nodes.forEach((node) => {
          const nodeEl = document.createElement("article");
          nodeEl.className = "cro-lineage-node";
          // 资源节点（aclOpExecutor / CCU mission）不是算子，用另一套描边区分：
          // 血缘图上「算子怎么变」与「资源被谁持有」是两件事，混成一种卡会读串。
          if (node.kind && node.kind !== "op") nodeEl.classList.add(`is-${node.kind}`);
    ...
    function renderIncidentRail() {
      ...
      INCIDENT_GROUPS.forEach((group) => {
        ...
        group.events.forEach((event, index) => {
          ...
          // 事件编号 = 组前缀.本组内序号，如「MC2.3」。训练版按「问题N」编号，
          // 本页只有一条链，用案例名当前缀比「问题1」更认得出来。
          event.code = `${group.codePrefix || `问题${group.id.replace("problem-", "")}`}.${index + 1}`;
