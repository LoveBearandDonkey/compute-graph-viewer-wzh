...
| A1 | [curM,curGmAKL1] | L1 | NZ | T | 动态 | 由当前输出 tile 和 L1 K tile 决定 |
| B1 | [curGmBKL1,curN] | L1 | NZ | T | 动态 | 由当前输出 tile 和 L1 K tile 决定 |
...
对于最后一个 M/N/K tile，A1、B1 的 shape 使用 `curM`、`curN`、`curGmBKL1`；最后一个 L0 K tile 使用 `curKL0`，不能继续按完整 tile shape 展示。
...
~~~
tensorAL1 · L1 · NZ · [curM,curGmAKL1]
