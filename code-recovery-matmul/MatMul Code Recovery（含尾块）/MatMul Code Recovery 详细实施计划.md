...
以上数值由 `CeilDiv`、`curM/curN`、`curGmBKL1` 和 `curKL0` 的源码公式派生；真实编译、运行、profiling 与 Tensor Data Dump 仍为 unverified。
