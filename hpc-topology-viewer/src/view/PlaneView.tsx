...
// 连线，下钻到 L1 Die / L0 Core-Group / Tile。仅显示选中(蓝圈)链路 + 关联对象。
...
export function PlaneView({ gen, dark, onSelect }: { gen: Gen; dark: boolean; onSelect?: (sel: PlaneSel) => void }) {
  ...
  const draw = useCallback(() => {
    ...
    if (layout === 'layers') {
      ...
      const glyph = (kind: string, x: number, y: number, ws: number, base: string, A: number) => {
        ...
        } else {   // L0 Core-Group = ONE independent core — Cube(cyan) or Vector(light cyan) via `base`; solid block
      ...
      levels.forEach((Lv, li) => {
        ...
        if (Lv.banner) {
          ...
          // "L4" chip (darker inset block on the left)
  ...
  const tipInfo = tip && (() => {
    ...
    if (tip.core !== undefined && tip.die !== undefined) {
      ...
      return [`AI Core（L0 Core-Group 成员 · ${vec ? 'AIV·Vector' : 'AIC·Cube'}）`, `卡 ${k} · 计算 Die ${tip.die} · core #${tip.core}（block_idx，SPMD）`, `${vec ? '矢量 Vector 单元' : '矩阵 Cube 单元'} · Cube∶Vector ≈ 8∶1 · 设备内并行（非 rank）`];
    }
      ...
      <div style={{ position: 'absolute', bottom: 12, left: 12, maxWidth: 'min(420px, calc(100vw - 24px))', padding: '7px 11px', fontSize: 11, background: 'var(--panel)', border: '1px solid var(--bd)', borderRadius: 10, boxShadow: 'var(--shadow-sm)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', lineHeight: 1.6, color: 'var(--tx2)' }}>
        ...
        {legendOpen && (layout === 'top' ? (
          ...
          <>
            ...
            <div style={{ ...lgNote, borderTop: '1px solid var(--bd)', marginTop: 3, paddingTop: 3 }}>逐级下探 L2 Chip→L1 Die→L0 Core-Group→Tile · <span style={{ color: ENTITY_COLORS.card }}>device</span>↔<span style={{ color: ENTITY_COLORS.rank }}>rank</span> 1:1</div>
