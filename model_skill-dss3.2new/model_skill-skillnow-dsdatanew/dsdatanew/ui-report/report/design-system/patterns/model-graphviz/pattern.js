(function registerPtoModelGraphvizPattern(global) {
  ...
  function applyPerformanceHeatmap(svg, graph, options = {}) {
    ...
    const candidates = (graph?.nodes || []).map((node) => ({ node, value: performanceHeatmapValue(node, resolvedOptions) }))
      .filter(({ node, value }) => getNodeVisualKind(node) === 'op'
        && node.dataState !== 'no_performance_data' && Number.isFinite(value));
  ...
  function edgePath(source, target, edge) {
    ...
    const sourceAnchorSide = typeof sourceAnchor === 'string' ? sourceAnchor : sourceAnchor?.side;
    const targetAnchorSide = typeof targetAnchor === 'string' ? targetAnchor : targetAnchor?.side;
    ...
    const verticalAnchors = ["top", "bottom"].includes(sourceAnchorSide)
      && ["top", "bottom"].includes(targetAnchorSide);
  ...
  function drawCluster(svg, cluster, color) {
    ...
    const group = createSvgElement('g', {
      class: `pto-model-graphviz-cluster${cluster.structuralRoot ? ' is-structural-root' : ''}${cluster.expertRole ? ` is-expert-${classToken(cluster.expertRole)}` : ''}${dataState ? ` is-${dataState}` : ''}${origin ? ` is-origin-${origin}` : ''}`,
    ...
    if (isRepeat && repeatCount > 1 && instanceIndices.length) {
      ...
      // Layer number and instance count are different quantities. The old form read
      // `Layer 3/5`, whose divisor was the LAST layer index while the dot row below it
      // showed instanceIndices.length dots -- so a group covering layers 3,4,5 rendered
      // "/5" against 3 dots, and a 58-layer group observed 3 times would read "/60".
      // Name the selected layer, then state the count the dots actually represent.
      pagerLabel.textContent = `${repeatLabel} · Layer ${selectedInstanceIndex}`
        + ` (${instanceIndices.indexOf(selectedInstanceIndex) + 1}/${instanceIndices.length})`;
  ...
  function drawNode(svg, node, color, options) {
    ...
    const extraClasses = [
      ...
      node.expertRole ? `is-expert-${classToken(node.expertRole)}` : '',
    ].filter(Boolean).join(' ');
    const group = createSvgElement('g', {
  ...
  function render(container, graph, options) {
    ...
    (data.edges || []).forEach((edge) => {
      ...
      // Fan-out/fan-in edges share one semantic fork/join point. Only edges projected
      // onto the exact same visible endpoints need port separation.
      const sourcePortOffset = edge.fixedPorts
        ? 0
        : portOffset(group, edge, source, spacing, verticalRoute);
      const targetPortOffset = edge.fixedPorts
        ? 0
        : portOffset(group, edge, targetNode, spacing, verticalRoute);
      const renderedEdge = {
        ...edge,
