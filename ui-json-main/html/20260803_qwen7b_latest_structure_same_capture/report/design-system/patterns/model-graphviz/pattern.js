(function registerPtoModelGraphvizPattern(global) {
  ...
  function performanceHeatmapValue(node, options = {}) {
    if (typeof options.value === 'function') return Number(options.value(node));
    const valueKey = options.valueKey || 'timeSharePct';
    const directValue = Number(node?.[valueKey] ?? node?.performance?.[valueKey] ?? node?.metrics?.[valueKey] ?? node?.report?.[valueKey]);
  ...
  function applyPerformanceHeatmap(svg, graph, options = {}) {
    ...
    const candidates = (graph?.nodes || []).map((node) => ({ node, value: performanceHeatmapValue(node, resolvedOptions) }))
    ...
    const maxValue = configuredMax > 0 ? configuredMax : Math.max(0, ...candidates.map(({ value }) => value));
  ...
  function edgePath(source, target, edge) {
    ...
    let start = edge?.sourcePoint
      ? { x: Number(edge.sourcePoint.x), y: Number(edge.sourcePoint.y) }
      : nodeAnchor(source, sourceAnchor);
    let end = edge?.targetPoint
      ? { x: Number(edge.targetPoint.x), y: Number(edge.targetPoint.y) }
      : nodeAnchor(target, targetAnchor);
    const parallelOffset = Number(edge?.parallelOffset);
    const sourcePortOffsetValue = Number(edge?.sourcePortOffset);
    const targetPortOffsetValue = Number(edge?.targetPortOffset);
    const sourcePortOffset = Number.isFinite(sourcePortOffsetValue) ? sourcePortOffsetValue : parallelOffset;
    const targetPortOffset = Number.isFinite(targetPortOffsetValue) ? targetPortOffsetValue : parallelOffset;
    const verticalAnchors = ["top", "bottom"].includes(sourceAnchor)
      && ["top", "bottom"].includes(targetAnchor);
    if (Number.isFinite(sourcePortOffset) && Math.abs(sourcePortOffset) >= 0.5) {
      start = verticalAnchors
        ? { ...start, x: start.x + sourcePortOffset }
        : { ...start, y: start.y + sourcePortOffset };
    }
    if (Number.isFinite(targetPortOffset) && Math.abs(targetPortOffset) >= 0.5) {
      end = verticalAnchors
        ? { ...end, x: end.x + targetPortOffset }
        : { ...end, y: end.y + targetPortOffset };
    }
  ...
  function drawCluster(svg, cluster, color) {
    ...
    const group = createSvgElement('g', {
      class: `pto-model-graphviz-cluster${cluster.structuralRoot ? ' is-structural-root' : ''}${dataState ? ` is-${dataState}` : ''}${origin ? ` is-origin-${origin}` : ''}`,
    ...
    if (isRepeat && repeatCount > 1 && instanceIndices.length) {
      ...
      const repeatLabel = String(cluster.label || cluster.id)
        ...
        .replace(/\s*[×x]\s*\d+\s*$/i, '');
      pagerLabel.textContent = `${repeatLabel} × ${repeatCount} Layers`;
      pager.appendChild(pagerLabel);

      const buttonSize = 36;
      const controlGap = 12;
      const dotStep = 24;
      const selectedSlotWidth = 88;
      const selectedPillWidth = 80;
      const dotsWidth = (instanceIndices.length - 1) * dotStep + selectedSlotWidth;
      ...
      instanceIndices.forEach((instanceIndex) => {
        ...
        const slotWidth = selected ? selectedSlotWidth : dotStep;
        const dotX = dotCursorX + slotWidth / 2;
        dotCursorX += slotWidth;
        const dot = createSvgElement('g', {
          ...
          'data-has-timing': hasData ? 'true' : 'false',
        ...
        if (selected) {
          dot.appendChild(createSvgElement('rect', {
            class: 'pto-model-graphviz-layer-dot-hit',
            x: dotX - selectedSlotWidth / 2,
            y: pagerControlsY - 18,
            width: selectedSlotWidth,
            height: 36,
            rx: 18,
            ry: 18,
          }));
          dot.appendChild(createSvgElement('rect', {
            class: 'pto-model-graphviz-layer-selected-pill',
            x: dotX - selectedPillWidth / 2,
            y: pagerControlsY - 16,
          ...
          const selectedLabel = createSvgElement('text', {
            class: 'pto-model-graphviz-layer-selected-label',
            x: dotX,
            y: pagerControlsY,
          });
          selectedLabel.textContent = `Layer ${instanceIndex}`;
          dot.appendChild(selectedLabel);
        } else {
  ...
  function drawMetricBadge(group, node) {
    ...
    const badgeHeight = 24;
    const badgeWidth = estimateTextWidth(metric, 44, 72) + 4;
    const repeatCount = Number(node.repeatCount || node.instanceIndices?.length || 0);
    const repeatWidth = node.collapsed && repeatCount > 1
      ? estimateTextWidth(`×${repeatCount}`, 36, 52) + 6
      : 0;
    const x = -node.width / 2 + 10 + repeatWidth;
    group.appendChild(createSvgElement('rect', {
  ...
  function drawNode(svg, node, color, options) {
    ...
    if (hasRepeatBadge) {
      ...
      const repeatWidth = estimateTextWidth(repeatLabel, 36, 52);
      group.appendChild(createSvgElement('rect', {
    ...
    const metricBadgeWidth = metricBadge ? estimateTextWidth(metricBadge, 44, 72) + 6 : 0;
    const repeatBadgeWidth = hasRepeatBadge ? estimateTextWidth(`×${repeatCount}`, 36, 52) + 6 : 0;
    const leftReserved = repeatBadgeWidth + metricBadgeWidth;
    const rightReserved = node.collapsed ? EXPAND_BUTTON_EDGE_GAP + EXPAND_BUTTON_RADIUS * 2 : 0;
    const contentX = (leftReserved - rightReserved) / 2;
    const label = createSvgElement('text', {
  ...
  function createController(stage, svg, graph, metadata, options) {
    ...
    function showHover(node, event) {
      ...
      hover.classList.remove('is-layer-tip');
    ...
    function hideHover() {
      if (!hover) return;
      hover.classList.remove('is-visible', 'is-layer-tip');
    ...
    clusterEntries.forEach(({ el, cluster, layerPager }) => {
      ...
      if (layerDots.length && typeof opts.onRepeatInstanceChange === 'function') {
        ...
        const showLayerHover = (dot, event) => {
          ...
          const clickHint = document.documentElement.lang.toLowerCase().startsWith('zh')
            ? '点击查看此 Layer'
            : 'Click to view this Layer';
          hover.innerHTML = [
            '<div class="pto-model-graphviz-layer-tip-summary">',
            `<strong>Layer ${esc(instanceIndex)}</strong>`,
            Number.isFinite(timeUs) ? `<span>${esc(timeUs.toFixed(2))} µs</span>` : '',
            '</div>',
            `<p class="pto-model-graphviz-layer-click-hint">${esc(clickHint)}</p>`,
          ].join('');
          hover.classList.add('is-visible', 'is-layer-tip');
          hover.setAttribute('aria-hidden', 'false');
          placeHover(stage, hover, event);
        };
  ...
  function render(container, graph, options) {
    ...
    const portOffset = (group, edge, node, spacing, vertical) => {
      const index = group.indexOf(edge);
      if (index < 0 || group.length < 2) return 0;
      const raw = (index - (group.length - 1) / 2) * spacing;
      const extent = vertical ? Number(node?.width) : Number(node?.height);
      const limit = Number.isFinite(extent) ? Math.max(0, extent / 2 - 18) : Math.abs(raw);
    ...
    (data.edges || []).forEach((edge) => {
      ...
      const group = parallelGroups.get(edgeKey) || [edge];
      const groupIndex = group.indexOf(edge);
      const spacingValue = Number(edge.parallelSpacing ?? resolvedOptions.parallelEdgeSpacing ?? 12);
      const spacing = Number.isFinite(spacingValue) ? Math.max(4, spacingValue) : 12;
      const automaticOffset = (groupIndex - (group.length - 1) / 2) * spacing;
      const explicitOffset = Number(edge.parallelOffset);
      const verticalRoute = (edge.curve || "vertical") === "vertical";
      const sourcePortOffset = edge.fixedPorts
        ? 0
        : portOffset(sourceGroups.get(edge.source) || [edge], edge, source, spacing, verticalRoute);
      const targetPortOffset = edge.fixedPorts
        ? 0
        : portOffset(targetGroups.get(edge.target) || [edge], edge, targetNode, spacing, verticalRoute);
      const renderedEdge = {
        ...edge,
        parallelOffset: Number.isFinite(explicitOffset) ? explicitOffset : automaticOffset,
        sourcePortOffset,
        targetPortOffset,
        parallelIndex: groupIndex,
        parallelCount: group.length,
      };
      const edgeType = edge.semanticEdgeType || edge.edgeType || edge.type || '';
      const el = createSvgElement('path', {
        class: `pto-model-graphviz-edge${edge.dataState ? ` is-${classToken(edge.dataState)}` : ''}${edgeType ? ` is-${classToken(edgeType)}` : ''}`,
        d: edgePath(source, targetNode, renderedEdge),
        stroke: edge.color || LINE_COLOR,
        'stroke-dasharray': edge.dashed ? '8 7' : null,
        'marker-end': `url(#${markerId})`,
        'data-edge-id': edge.id || null,
        'data-source': edge.source,
        'data-target': edge.target,
        'data-edge-type': edgeType || null,
        'data-parallel-index': groupIndex,
        'data-parallel-count': group.length,
        'data-source-port-offset': sourcePortOffset,
        'data-target-port-offset': targetPortOffset,
      });
      svg.appendChild(el);
      edgeEntries.push({ el, edge: renderedEdge, source: edge.source, target: edge.target, sourceNode: source, targetNode, avoidNodes: data.nodes, avoidClusters: data.clusters, tagEl: null });
    });
    ...
    const performanceHeatmap = applyPerformanceHeatmap(svg, data, resolvedOptions.performanceHeatmap);
